...
class LMModel(StreamingModule):
    ...
    def __init__(self, pattern_provider: CodebooksPatternProvider, condition_provider: ConditioningProvider,
        ...
        self.emb = nn.ModuleList([nn.Linear(128, dim)])
    ...
    def forward(self, sequence: torch.Tensor,
                ...
                embedding_model = None) -> torch.Tensor:
        ...
        if embedding_model is not None:
            with torch.no_grad():
                emb = embedding_model.quantizer.decode(sequence)
            for layer in self.emb:        
                input_ = layer(emb.permute(0,2,1)) 
        else:
    ...
    def compute_predictions(
        ...
        logits = model(sequence_codes, conditions, condition_tensors, embedding_model)  # [B, K, S, card]
    ...
    def _sample_next_token(self,
        ...
        if two_step_cfg and cfg_conditions != {}:
            ...
            uncond_logits = model(sequence, conditions=[], condition_tensors=null_condition_tensors, embedding_model=embedding_model)
            ...
            all_logits = model(
                sequence,
                conditions=[], condition_tensors=condition_tensors, embedding_model=embedding_model)
            if condition_tensors:
        ...
        with self.streaming():
            ...
            for offset in range(start_offset_sequence, gen_sequence_len):
                ...
                next_token = self._sample_next_token(
                    ...
                    cfg_coef=cfg_coef, two_step_cfg=two_step_cfg, embedding_model=embedding_model)
