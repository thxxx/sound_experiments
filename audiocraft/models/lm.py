...
class LMModel(StreamingModule):
    ...
    @torch.no_grad()
    def generate_audio_inpainting(self,
                 prompt: tp.Optional[torch.Tensor] = None,
                 conditions: tp.List[ConditioningAttributes] = [],
        ...
        if audio_conditions:
            b = cfg_conditions['description'][0].size(0)
            #print(b)
            expanded_audio_conditions = (audio_conditions[0].repeat(b, 1, 1), audio_conditions[1])
            cfg_conditions["sound"] = expanded_audio_conditions

        #print(cfg_conditions['description'][0].shape)
        #print(cfg_conditions['sound'][0].shape)
